...
import com.library.security.ApiRateLimitFilter;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/swagger-ui/**", "/v3/api-docs/**", "/doc.html", "/webjars/**").hasRole("ADMIN")
                        .requestMatchers("/actuator/health").hasAnyRole("ADMIN", "LIBRARIAN")
                        .requestMatchers("/actuator/**").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.GET, "/book-resources/download/**").authenticated()
                        ...
                        .requestMatchers("/export/**").hasAnyRole("ADMIN", "LIBRARIAN")
                        .requestMatchers("/operation-logs/**").hasRole("ADMIN")
                        .requestMatchers("/depts/**").authenticated()
                        .anyRequest().authenticated())
                .addFilterBefore(loginRateLimitFilter, UsernamePasswordAuthenticationFilter.class)
                .addFilterBefore(apiRateLimitFilter, LoginRateLimitFilter.class)
                .addFilterBefore(jwtAuthenticationFilter, ApiRateLimitFilter.class)
                .addFilterBefore(xssSqlFilter, JwtAuthenticationFilter.class)
                .addFilterBefore(securityHeadersFilter, XssSqlFilter.class)
                .addFilterBefore(requestValidationFilter, SecurityHeadersFilter.class);
    ...
    @Bean
    public FilterRegistrationBean<RequestValidationFilter> requestValidationFilterRegistration(RequestValidationFilter filter) {
        FilterRegistrationBean<RequestValidationFilter> registration = new FilterRegistrationBean<>(filter);
        registration.setEnabled(false);
        return registration;
    }
