...
import { exportBooksExcel, exportBooksCsv, downloadBlob } from '@/api/export'
...
function handleExport(command: string) {
  ...
  if (command === 'excel') {
    exportBooksExcel().then((blob: any) => {
      downloadBlob(blob, `图书数据_${new Date().toISOString().slice(0, 10)}.xlsx`)
      ElMessage.success('Excel 导出成功')
    }).catch(() => ElMessage.error('导出失败'))
  } else if (command === 'csv') {
    exportBooksCsv().then((blob: any) => {
      downloadBlob(blob, `图书数据_${new Date().toISOString().slice(0, 10)}.csv`)
