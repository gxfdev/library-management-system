...
import { getSecureToken, clearAuthData } from '@/api/request'
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        meta: { title: '高级报表', icon: 'TrendCharts', roles: ['ADMIN', 'LIBRARIAN'] },
      },
      {
        path: 'audit-log',
        name: 'AuditLog',
        component: () => import('@/views/audit-log/index.vue'),
        meta: { title: '操作日志', icon: 'List', roles: ['ADMIN'] },
      },
      {
        path: 'publishers',
...
router.beforeEach(async (to, _from, next) => {
  ...
  const token = userStore.token || getSecureToken()
  ...
  if (!userStore.userInfo && !userInfoFetched) {
    try {
      await userStore.initUserInfo(true)
