<template>
  <div class="app-layout" :class="{ 'mobile': isMobile, 'sidebar-open': isMobile && !isCollapsed }">
    <div v-if="isMobile && !isCollapsed" class="sidebar-overlay" @click="isCollapsed = true"></div>
    <aside class="sidebar" :class="{ collapsed: isCollapsed, 'mobile-sidebar': isMobile }">
      <div class="sidebar-inner">
        ...
        <nav class="sidebar-nav">
          <div class="nav-group">
            ...
            <router-link to="/dashboard" class="nav-item" :class="{ active: route.path === '/dashboard' }" @click="onMobileNavClick">
              <el-icon><Odometer /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">首页看板</span></transition>
              <span v-if="!isCollapsed" class="nav-badge home-badge"></span>
            </router-link>

            <router-link to="/book-catalog" class="nav-item" :class="{ active: route.path === '/book-catalog' }" @click="onMobileNavClick">
              <el-icon><Reading /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">图书借阅</span></transition>
            </router-link>

            <router-link v-if="isAdmin || isLibrarian" to="/books" class="nav-item" :class="{ active: route.path === '/books' }" @click="onMobileNavClick">
              <el-icon><Notebook /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">图书管理</span></transition>
            </router-link>

            <router-link v-if="isAdmin" to="/users" class="nav-item" :class="{ active: route.path === '/users' }" @click="onMobileNavClick">
              <el-icon><User /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">用户管理</span></transition>
            </router-link>

            <router-link v-if="isAdmin || isLibrarian" to="/categories" class="nav-item" :class="{ active: route.path === '/categories' }" @click="onMobileNavClick">
              <el-icon><Menu /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">分类管理</span></transition>
            </router-link>

            <router-link v-if="isAdmin || isLibrarian" to="/borrows" class="nav-item" :class="{ active: route.path === '/borrows' }" @click="onMobileNavClick">
              <el-icon><Tickets /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">借阅管理</span></transition>
            </router-link>

            <router-link v-if="isReader" to="/my-borrows" class="nav-item" :class="{ active: route.path === '/my-borrows' }" @click="onMobileNavClick">
              <el-icon><DocumentChecked /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">我的借阅</span></transition>
            </router-link>

            <router-link v-if="isAdmin || isLibrarian" to="/inventory" class="nav-item" :class="{ active: route.path === '/inventory' }" @click="onMobileNavClick">
              <el-icon><Box /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">库存管理</span></transition>
            </router-link>

            <router-link v-if="isAdmin || isLibrarian" to="/statistics" class="nav-item" :class="{ active: route.path === '/statistics' }" @click="onMobileNavClick">
            ...
            <router-link v-if="isAdmin || isLibrarian" to="/publishers" class="nav-item" :class="{ active: route.path === '/publishers' }" @click="onMobileNavClick">
              <el-icon><OfficeBuilding /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">出版社管理</span></transition>
            </router-link>

            <router-link v-if="isAdmin || isLibrarian" to="/bookshelves" class="nav-item" :class="{ active: route.path === '/bookshelves' }" @click="onMobileNavClick">
              <el-icon><Grid /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">书架管理</span></transition>
            </router-link>

            <router-link v-if="isAdmin || isLibrarian" to="/purchases" class="nav-item" :class="{ active: route.path === '/purchases' }" @click="onMobileNavClick">
              <el-icon><ShoppingCart /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">采购管理</span></transition>
            </router-link>

            <router-link v-if="isAdmin || isLibrarian" to="/book-normalize" class="nav-item" :class="{ active: route.path === '/book-normalize' }" @click="onMobileNavClick">
              <el-icon><EditPen /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">图书规范化</span></transition>
            </router-link>

            <router-link to="/notices" class="nav-item" :class="{ active: route.path === '/notices' }" @click="onMobileNavClick">
              <el-icon><Bell /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">通知公告</span></transition>
            </router-link>

            <router-link to="/borrow-code" class="nav-item" :class="{ active: route.path === '/borrow-code' }" @click="onMobileNavClick">
              <el-icon><Key /></el-icon>
              <transition name="fade"><span v-if="!isCollapsed">借阅码</span></transition>
            </router-link>

            <router-link v-if="isAdmin" to="/permissions" class="nav-item" :class="{ active: route.path === '/permissions' }" @click="onMobileNavClick">
